...
(function () {
    ...
    var Joi = require('joi'),
        ...
        internals = {
            const: {
                MISSING_PARAM: 'Param missing',
                MISSING_EXAMPLE: 'Example is missing'
            },
            testValues: []
        };
    ...
    internals.getCoverageTests = function (route, userTests, validations, combos) {
        var method = route.method,
            pathSpec = route.path,
            tests = []; // Tests passed back to the API
        ...
        combos.forEach(function (combo) {
            ...
            Object.keys(combo).forEach(function (paramType) {
                ...
                test.request[paramType] = {};

                // Lastly we loop through each param details object
                combo[paramType].forEach(function (paramSpec) {

                    if(!paramSpec.examples || paramSpec.examples.length === 0) {
                        test.issues.push({
                            prop: paramSpec.name,
                            propType: paramType,
                            reason: internals.const.MISSING_EXAMPLE
                        });
                    } else {
                        test.request[paramType][paramSpec.name] = paramSpec.examples[0];
                    }

                    userTests.forEach(function(paramValues) {
                        if(paramValues[paramSpec.name] === undefined) {

                            test.issues.push({
                                prop: paramSpec.name,
                                propType: paramType,
                                reason: internals.const.MISSING_PARAM
                            });
    ...
    internals.filteredTestsUsingJoi = function (tests) {
        ...
        tests = tests.filter(function (test) {
            ...
            if (test.request.payload &&
                Object.keys(test.request.payload).length > 0 ||
